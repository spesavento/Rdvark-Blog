...
SELECT COUNT(DISTINCT id) AS teacher_n, COUNT(mobile) AS mobile_n FROM teacher;
...
SELECT dept.name, COUNT(teacher.id) AS teacher_n FROM teacher RIGHT JOIN dept
ON(teacher.dept = dept.id)
GROUP BY(dept.name);

-- 9. Use CASE to show the name of each teacher followed by 'Sci' if the teacher is in dept 1 or 2 and 'Art'
-- otherwise.
SELECT name, CASE WHEN dept = 1 OR dept = 2 THEN 'Sci' ELSE 'Art' END AS dept_type FROM teacher;
...
SELECT name, CASE WHEN dept = 1 OR dept = 2 THEN 'Sci' WHEN dept = 3 THEN 'Art'
ELSE 'None' END AS dept_type FROM teacher;
