...
WHERE question = 'Q22'
GROUP BY subject HAVING subject IN('(8) Computer Science', '(H) Creative Arts
and Design');

-- 5. Show the subject and total number of students who A_STRONGLY_AGREE to question 22 for each of the
...
-- 6. Show the percentage of students who A_STRONGLY_AGREE to question 22 for the subject '(8) Computer
-- Science' show the same figure for the subject '(H) Creative Arts and Design'. Use the ROUND function to show the
-- percentage without decimal places.
SELECT subject, ROUND(SUM(A_STRONGLY_AGREE * response) / SUM(response)) AS
percent_strong_agree FROM nss
WHERE question='Q22'
GROUP BY subject HAVING subject IN('(8) Computer Science', '(H) Creative Arts
and Design');
...
-- Manchester for 'Q01'.
SELECT institution, SUM(sample) AS sample_n, SUM(CASE WHEN subject = '(8)
Computer Science' THEN sample ELSE 0 END) AS computing_n FROM nss
